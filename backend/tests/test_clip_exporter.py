...
class TestBuildClipFilename:
    def test_format(self):
        result = build_clip_filename("game.mp4", 1, 5.0, 15.5, "score")
        assert result == "game_clip_001_score_5.0s-15.5s.mp4"

    def test_strips_extension(self):
        result = build_clip_filename("video.mov", 2, 10.0, 20.0, "near_miss")
        assert result == "video_clip_002_near_miss_10.0s-20.0s.mp4"

    def test_index_padding(self):
        result = build_clip_filename("test.mp4", 10, 0.0, 5.0, "score")
        assert result == "test_clip_010_score_0.0s-5.0s.mp4"

    def test_decimal_formatting(self):
        result = build_clip_filename("clip.mp4", 1, 1.234, 9.876, "score")
        assert result == "clip_clip_001_score_1.2s-9.9s.mp4"

    def test_near_miss_in_filename(self):
        result = build_clip_filename("game.mp4", 3, 5.0, 15.0, "near_miss")
...
class TestExportClips:
    ...
    def test_calls_ffmpeg_for_each_segment(self, mock_ffmpeg, mock_getsize, tmp_path):
        ...
        mock_chain.trim.return_value = mock_chain
        mock_chain.setpts.return_value = mock_chain
        mock_chain.output.return_value = mock_chain
        mock_chain.overwrite_output.return_value = mock_chain

        segments = [(0.0, 5.0, "score"), (10.0, 20.0, "near_miss"), (30.0, 45.0, "score")]
    ...
    def test_correct_parameters(self, mock_ffmpeg, mock_getsize, tmp_path):
        ...
        export_clips("/fake/input.mp4", [(5.0, 15.0, "score")], str(tmp_path), "game.mp4")

        mock_ffmpeg.input.assert_called_once_with("/fake/input.mp4")
        mock_chain.trim.assert_called_once_with(start=5.0, end=15.0)
        mock_chain.setpts.assert_called_once_with("PTS-STARTPTS")
        expected_path = os.path.join(str(tmp_path), "game_clip_001_score_5.0s-15.0s.mp4")
    ...
    def test_returns_correct_metadata(self, mock_ffmpeg, mock_getsize, tmp_path):
        ...
        result = export_clips("/fake/input.mp4", [(5.0, 15.0, "score")], str(tmp_path), "game.mp4")

        assert len(result) == 1
        clip = result[0]
        assert clip.filename == "game_clip_001_score_5.0s-15.0s.mp4"
        assert clip.source_video == "game.mp4"
        assert clip.start_time == 5.0
        assert clip.end_time == 15.0
        assert clip.duration == 10.0
        assert clip.size_mb == 2.0
        assert clip.event_type == "score"
    ...
    def test_url_format(self, mock_ffmpeg, mock_getsize, tmp_path):
        ...
        result = export_clips("/fake/input.mp4", [(0.0, 5.0, "score")], str(tmp_path), "game.mp4")

        assert result[0].url == "/output/game_clip_001_score_0.0s-5.0s.mp4"
    ...
    def test_creates_output_dir(self, mock_ffmpeg, mock_getsize, tmp_path):
        ...
        export_clips("/fake/input.mp4", [(0.0, 5.0, "score")], output_dir, "game.mp4")
    ...
    def test_ffmpeg_error(self, mock_ffmpeg, tmp_path):
        ...
        with pytest.raises(RuntimeError, match="ffmpeg failed cutting"):
            export_clips("/fake/input.mp4", [(0.0, 5.0, "score")], str(tmp_path), "game.mp4")
    ...
    @patch("services.clip_exporter.os.path.getsize", return_value=1024)
    @patch("services.clip_exporter.ffmpeg")
    def test_event_type_in_output(self, mock_ffmpeg, mock_getsize, tmp_path):
        mock_chain = MagicMock()
        mock_ffmpeg.input.return_value = mock_chain
        mock_chain.trim.return_value = mock_chain
        mock_chain.setpts.return_value = mock_chain
        ...
        result = export_clips("/fake/input.mp4", [(0.0, 5.0, "near_miss")], str(tmp_path), "game.mp4")

        assert result[0].event_type == "near_miss"
        assert "near_miss" in result[0].filename
